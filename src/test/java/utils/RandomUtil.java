...
public class RandomUtil {
    ...
    public static void printStats(int itr, long op, long ns, int hits) {
        if (itr == 1) {
            System.out.printf("%-10s %-15s %-20s %-15s %-15s%n", "Iteration", "Duration", "Operation", "Throughput", "Hits");
        }
        System.out.printf("%-10d %-15s %-20s %-15s %-15s%n", itr, nsToMilliseconds(ns) + " ms", formatNumber((double) ns / op) + " ns/op", formatNumber((double) op / nsToSeconds(ns)) + " op/sec", hits*100.0/(float)op+" %");
    }
}
